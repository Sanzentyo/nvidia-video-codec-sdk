...
impl PracticalRealtimeRecorder {
    ...
    fn start_recording(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        // 出力スレッド
        let output_handle =
            self.start_output_thread(Arc::clone(&stats), Arc::clone(&running), encoded_rx)?;

        // 統計表示スレッド
        let stats_handle = self.start_stats_thread(Arc::clone(&stats), Arc::clone(&running));

        println!(
            "Recording started ({}x{} @ {}fps). Recording for 10 seconds...",
                ...
                match session.encode_picture(input_buffer, output_buffer, Default::default()) {
                    Ok(_) => {
                        ...
                        let lock = output_buffer
                            .lock()
                            .expect("Output buffer should be lockable");
                ...
                match out_file.write_all(&encoded_frame.data) {
                    Ok(_) => {
                        ...
                        if encoded_frame.frame_index % 30 == 0 {
                            println!(
                                "Written frame {} ({})",
                                encoded_frame.frame_index, encoded_frame.picture_type
                            );
                        }
                    }
                    Err(e) => {
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut recorder =
        PracticalRealtimeRecorder::new(1920, 1080, 30, "realtime_output.bin".to_string());
